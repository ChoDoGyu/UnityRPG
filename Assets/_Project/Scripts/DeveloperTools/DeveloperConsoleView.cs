...
namespace UnityRPG.DeveloperTools
{
    ...
    public sealed class DeveloperConsoleView : MonoBehaviour
    {
        ...
        private bool clearToggleCharacter;
        ...
        private void OnGUI()
        {
            ...
            if (currentEvent.type == EventType.KeyDown && IsConsoleToggleEvent(currentEvent))
            {
                currentEvent.Use();
                return;
            }
            ...
            GUI.Box(
                new Rect(0f, panelY, screenWidth, panelHeight),
                GUIContent.none);

            GUI.Label(
                new Rect(margin, panelY + margin, historyWidth, titleHeight),
                "Developer Console [`]",
                titleStyle);
            ...
            GUI.Label(
                new Rect(margin, historyY, historyWidth, historyHeight),
                historyText,
                historyStyle);
            ...
            input = GUI.TextField(
                new Rect(margin, inputY, historyWidth, inputHeight),
                input,
                inputStyle);

            if (clearToggleCharacter)
            {
                input = input.Replace("`", string.Empty).Replace("~", string.Empty);

                if (currentEvent.type == EventType.Repaint)
                    clearToggleCharacter = false;
            }
        ...
        private bool IsConsoleToggleEvent(Event currentEvent)
        {
            return currentEvent.keyCode == KeyCode.BackQuote ||
        ...
        private string BuildVisibleHistoryText(float width, float height)
        {
            ...
            for (int startIndex = 0; startIndex < history.Count; startIndex++)
            {
                ...
                float requiredHeight =
                    historyStyle.CalcHeight(new GUIContent(text), width);
